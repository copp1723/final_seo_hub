...
// with mocks for Prisma, NextAuth, etc.import { GET, POST, PUT, DELETE } from './route' // Adjust import path as needed
...
describe('API Route: /api/admin/agencies/[agencyId]/users', () => {
  ...
  describe('GET /users', () => {
    it('should return users for SUPER_ADMIN', async () => {
      (prisma.users.findMany as jest.Mock).mockResolvedValue([{ id: '1', name: 'Test User' }])
  ...
  describe('POST /users', () => {
    ...
    it('should allow SUPER_ADMIN to create user', async () => {
      ...
      (prisma.users.create as jest.Mock).mockResolvedValue({ ...users.ata, id: 'new-id', agencyId: params.agencies?.id })
      ...
      expect(prisma.users.create).toHaveBeenCalledWith(expect.objectContaining({
        data: expect.objectContaining({ ...users.ata, agencyId: params.agencies?.id })
      }))
    })

    it('should allow AGENCY_ADMIN to create USER or AGENCY_ADMIN role user', async () => {
      (prisma.users.findUnique as jest.Mock).mockResolvedValue(null)
      ;(prisma.users.create as jest.Mock).mockResolvedValue({ ...users.ata, role: UserRole.AGENCY_ADMIN, id: 'new-id', agencyId: params.agencies?.id })
      const req = mockRequest('POST', { ...users.ata, role: UserRole.AGENCY_ADMIN }, undefined, mockAgencyAdmin)
      const response = await POST(req, { params })
      expect(response.status).toBe(201)
    })

    it('should prevent AGENCY_ADMIN from creating SUPER_ADMIN role user', async () => {
      const req = mockRequest('POST', { ...users.ata, role: UserRole.SUPER_ADMIN }, undefined, mockAgencyAdmin)
      const response = await POST(req, { params })
      expect(response.status).toBe(403)
    })

    it('should return 409 if user email already exists', async () => {
      (prisma.users.findUnique as jest.Mock).mockResolvedValue({ id: 'existing-id', email: userData.email });
      const req = mockRequest('POST', userData, undefined, mockSuperAdmin)
      const response = await POST(req, { params })
  ...
  describe('PUT /users', () => {
    ...
    const existingUserInAgency = { id: 'user-to-update-id', name: 'Old Name', email: 'user@agency.com', role: UserRole.USER, agencyId: params.agencies?.id }

    it('should allow SUPER_ADMIN to update user', async () => {
      (prisma.users.findUnique as jest.Mock).mockResolvedValue(existingUserInAgency)
      ;(prisma.users.update as jest.Mock).mockResolvedValue({ ...existingUserInAgency, ...updateData })
    ...
    it('should allow AGENCY_ADMIN to update user in their agency (e.g USER to AGENCY_ADMIN)', async () => {
      ...
      ;(prisma.users.findUnique as jest.Mock).mockResolvedValue(existingUserInAgency)
      ;(prisma.users.update as jest.Mock).mockResolvedValue({ ...existingUserInAgency, ...agencies.dminUpdateData })
    ...
    it('should prevent AGENCY_ADMIN from updating user in another agency', async () => {
        (prisma.users.findUnique as jest.Mock).mockResolvedValue({ ...existingUserInAgency, agencyId: 'another-agency' })
  ...
  describe('DELETE /users', () => {
    const userToDelete = { id: 'user-to-delete-id', name: 'User To Delete', email: 'delete@agency.com', role: UserRole.USER, agencyId: params.agencies?.id }

    it('should allow SUPER_ADMIN to delete user', async () => {
      (prisma.users.findUnique as jest.Mock).mockResolvedValue(userToDelete)
      ;(prisma.users.delete as jest.Mock).mockResolvedValue(userToDelete)
    ...
    it('should prevent deleting oneself', async () => {
      (prisma.users.findUnique as jest.Mock).mockResolvedValue(mockAgencyAdmin) // User to delete is the admin themselves
    ...
    it('should prevent AGENCY_ADMIN from deleting SUPER_ADMIN', async () => {
      const superAdminUser = { ...users.oDelete, role: UserRole.SUPER_ADMIN }
      ;(prisma.users.findUnique as jest.Mock).mockResolvedValue(superAdminUser)
    ...
    it('should return 404 if user to delete not found in agency', async () => {
        (prisma.users.findUnique as jest.Mock).mockResolvedValue({ ...users.oDelete, agencyId: 'another-agency'})
        const searchParams = new URLSearchParams({ userId: userToDelete.id })
        const req = mockRequest('DELETE', undefined, searchParams, mockAgencyAdmin)
        const response = await DELETE(req, { params })
