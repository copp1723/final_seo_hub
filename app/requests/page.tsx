...
export default function RequestsPage() {
  ...
  // Client-side filtering is no longer needed as it's handled by the API
  // const filteredRequests = requests.filter(request => { ... })

  // For display purposes, we still might want to filter out requests without packages if that's a business rule
  // However, the API should ideally handle this if it's a hard requirement.
  // For now, let's assume the API returns all user's requests and we display them.
  // If a specific filter for "has package" is needed, it should be added to API and UI.
  const displayRequests = requests; // Using 'requests' directly from state
      ...
      {!loading && displayRequests.length === 0 ? (
        ...
        <div className="space-y-6">
          <div className="text-sm text-gray-600">
            {/* Ensure `requests` here refers to the full unfiltered list if you want to show total count before filtering */}
            {/* For now, it shows the count of items returned by the API based on current filters */}
